...
{
    public class PersonalComparisonExportGenerator(IPersonalExportRepository repository, IOptions<ReportConfig> reportConfig) : IReportGenerator
    {
        ...
        public async Task<ExportResultModel> GenerateReportAsync(ExportRequestBaseModel model)
        {
            ...
            if (model is PersonalExportRequest requestModel)
            {
                ...
                // 建立個人健康檢查文件並產生PDF
                var document = new PersonalComparisonReportDocument(personalHealthExam, _reportConfig);
