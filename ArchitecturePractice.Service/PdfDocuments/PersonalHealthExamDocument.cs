...
namespace ArchitecturePractice.Services.PdfDocuments
{
    ...
    /// </summary>
    internal class PersonalHealthExamDocument : IDocument
    {
        private readonly PersonalHealthExamModel _model;

        private readonly ReportConfig _reportConfig;

        // DI注入，除了所需Model外，還有報告配置參數(醫院資訊、異常評直陣列)
        public PersonalHealthExamDocument(PersonalHealthExamModel model, ReportConfig reportConfig)
        {
            _model = model;
